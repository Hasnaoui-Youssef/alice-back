...
export class ProductService {
  constructor(
    ...
    @InjectModel(ClientReview.name)
    private readonly clientReviewModel: Model<ClientReview>
  ) {}
  async createProduct(createProductDTO: CreateProductDTO) {
    try {
      if (typeof createProductDTO.category === "string") {
        createProductDTO.category = new Types.ObjectId(
  ...
  async getAllProducts() {
    return await this.productModel
      .find()
      .populate<{ category: string }>("category");
  }
  ...
  async addProductQuantity(productId: string, quantity: number) {
    try {
      ...
      throw new BadRequestException(
        "Unable to add product quantity ",
        error.message
      );
    }
  }
  ...
  async createClientReview(createClientReviewDTO: CreateClientReviewDTO) {
    try {
      createClientReviewDTO.productId = new Types.ObjectId(
        createClientReviewDTO.productId
      );
      createClientReviewDTO.clientId = new Types.ObjectId(
        createClientReviewDTO.clientId
      );
      const product = await this.productModel.findById(
        createClientReviewDTO.productId
      );
      ...
      const newRating =
        (totalRating + createClientReviewDTO.rating) /
  ...
  async updateClientReview(
    ...
    try {
      return await this.clientReviewModel.findByIdAndUpdate(
        new Types.ObjectId(crId),
  ...
  async deleteClientReview(crId: string) {
    try {
      const clientReview = await this.clientReviewModel.findByIdAndDelete(
        new Types.ObjectId(crId)
      );
      const product = await this.productModel.findById(clientReview.productId);
      const totalRating = product.averageRating * product.reviewsNumber;
      const newRating =
        (totalRating - clientReview.rating) / (product.reviewsNumber - 1);
      product.averageRating = newRating;
      product.reviewsNumber -= 1;
      await product.save();
      return clientReview;
  ...
  async getProductReviews(productId: string) {
    try {
      ...
      throw new BadRequestException(
        "Cannot find reviews for product ",
        error.message
      );
    }
  }
  async deleteProductReviews(productId: Types.ObjectId) {
    try {
      ...
      throw new BadRequestException(
        "Cannot delete reviews for product ",
        error.message
      );
    }
  }
